import { type AnyNode, BlockNode, createBoxBlockTopology, ZoneNode } from '@pascal-app/core'
...
function hull(points: PlanPoint[]): PlanPoint[] {
  ...
  const turn = (a: PlanPoint, b: PlanPoint, c: PlanPoint) =>
    (b[0] - a[0]) * (c[1] - a[1]) - (b[1] - a[1]) * (c[0] - a[0])
  const half = (list: PlanPoint[]) => {
...
function roomName(text: string): string | null {
  ...
  if (
    !/\b(bed\s*room|kitchen|dining|living|bath|toilet|porch|office|garage|hall|balcony|laundry|storage|wc)\b|\bT\s*[&+]\s*B\b/i.test(
      clean,
    )
  )
    return null
...
export function addReconstructionContext(
  nodes: Record<string, AnyNode>,
  levelId: string,
  variant: ComparisonVariant,
  source: { width: number; height: number; rgba: Uint8Array; labels?: SourceLabel[] },
  options: NativePreviewOptions,
) {
  const level = nodes[levelId]
  if (!level || level.type !== 'level')
    throw new Error('Reconstruction context requires its isolated native level')
  ...
  const scale = Math.min(1, 900 / Math.max(source.width, source.height)),
    mpp = options.metersPerPixel
  const width = Math.ceil(source.width * scale),
    height = Math.ceil(source.height * scale),
    size = width * height
  const footprint = hull(
    variant.candidates
      .filter((c) => c.decision?.correctedClass !== 'background')
      .flatMap((c) => c.outer),
  )
  const owner = new Int32Array(size).fill(-1),
    wallOwner = new Int32Array(size).fill(-1)
  const queue = new Int32Array(size)
  for (let y = 0; y < height; y++)
    for (let x = 0; x < width; x++)
      if (!inside((x + 0.5) / scale, (y + 0.5) / scale, footprint)) owner[y * width + x] = -2
  walls.forEach((wall, index) => {
    const a = wall.start.map((v) => (v / mpp) * scale),
      b = wall.end.map((v) => (v / mpp) * scale)
    const dx = b[0]! - a[0]!,
      dy = b[1]! - a[1]!,
      length2 = dx * dx + dy * dy
    const radius = Math.max(1, (((wall.thickness ?? 0.1) / mpp) * scale) / 2)
    for (
      let y = Math.max(0, Math.floor(Math.min(a[1]!, b[1]!) - radius));
      y <= Math.min(height - 1, Math.ceil(Math.max(a[1]!, b[1]!) + radius));
      y++
    )
      for (
        let x = Math.max(0, Math.floor(Math.min(a[0]!, b[0]!) - radius));
        x <= Math.min(width - 1, Math.ceil(Math.max(a[0]!, b[0]!) + radius));
        x++
      ) {
        const t = Math.max(
          0,
          Math.min(1, ((x + 0.5 - a[0]!) * dx + (y + 0.5 - a[1]!) * dy) / length2),
        )
        if (Math.hypot(x + 0.5 - a[0]! - t * dx, y + 0.5 - a[1]! - t * dy) <= radius) {
          owner[y * width + x] = -2
          wallOwner[y * width + x] = index
        }
      }
  })
  ...
  for (const label of source.labels ?? []) {
    ...
    if (!name || label.confidence < 0.4) continue
    const cx = Math.floor(((label.box[0] + label.box[2]) / 2) * scale),
      cy = Math.floor(((label.box[1] + label.box[3]) / 2) * scale)
    let seed = -1
    for (let radius = 0; radius <= 10 && seed < 0; radius++)
      for (let dy = -radius; dy <= radius && seed < 0; dy++)
        for (let dx = -radius; dx <= radius; dx++) {
          const x = cx + dx,
            y = cy + dy
          if (x >= 0 && x < width && y >= 0 && y < height && owner[y * width + x] === -1) {
            seed = y * width + x
            break
          }
        }
  ...
  const boundaries = names.map(() => new Map<number, number>()),
    counts = new Uint32Array(names.length)
  const adjacentWalls = names.map(() => new Set<number>()),
    open = new Set<number>()
  ...
  for (let y = 0; y < height; y++)
    for (let x = 0; x < width; x++) {
      ...
      const edges = [
        [corner, corner + 1],
        [corner + 1, corner + cornerWidth + 1],
        [corner + cornerWidth + 1, corner + cornerWidth],
        [corner + cornerWidth, corner],
      ]
  ...
  for (let id = 0; id < names.length; id++) {
    if (counts[id]! * (mpp / scale) ** 2 < 0.5 || (names[id]!.inferred && open.has(id))) continue
    ...
    while (edges.size) {
      ...
      while (point !== undefined && edges.has(point)) {
        ring.push([
          ((point % cornerWidth) / scale) * mpp,
          (Math.floor(point / cornerWidth) / scale) * mpp,
        ])
        const next: number | undefined = edges.get(point)
        edges.delete(point)
        point = next
    ...
    polygon = simplify([...polygon, polygon[0]!], Math.max(0.02, mpp * 1.5)).slice(0, -1)
    if (polygon.length < 3) continue
    ...
    const zone = ZoneNode.parse({
      ...
      polygon,
      autoFromWalls: false,
      boundaryWallIds: [...adjacentWalls[id]!].map((i) => walls[i]!.id),
      spaceRole: 'room',
      enclosureStatus: open.has(id) ? 'open' : 'enclosed',
      ceilingHeight: options.wallHeight,
      color: ['#285f46', '#315b83', '#75531e', '#67447f'][id % 4],
      metadata: {
        nativeMaskPreview: {
          role: 'zone',
          derivation: 'wall-bounded-source-label-region',
          approximation: true,
          nameSource: evidence.label ? 'offline-source-ocr' : 'unlabelled-enclosure',
          sourceLabel: evidence.label ?? null,
          boundaryPolicy: open.has(id)
            ? 'approximate-open-plan-division'
            : 'reconstructed-wall-enclosure',
        },
      },
    })
    nodes[zone.id] = zone
    level.children.push(zone.id)
    zones.push(zone)
    zoneByRegion.set(id, zone)
  }

  // Interior source-ink components provide prop footprints. Semantic review supplies only the object class.
  const labels = new Int32Array(source.width * source.height).fill(-1),
    componentQueue = new Int32Array(labels.length)
  const components: { minX: number; minY: number; maxX: number; maxY: number; count: number }[] = []
  const textBoxes = source.labels ?? []
  const pixelRegion = (x: number, y: number) =>
    owner[
      Math.min(height - 1, Math.floor(y * scale)) * width +
        Math.min(width - 1, Math.floor(x * scale))
    ] ?? -2
  for (let y = 0; y < source.height; y++)
    for (let x = 0; x < source.width; x++) {
      ...
      if (
        luminance > 190 ||
        !zoneByRegion.has(pixelRegion(x, y)) ||
        textBoxes.some(
          (t) => x >= t.box[0] - 2 && x <= t.box[2] + 2 && y >= t.box[1] - 2 && y <= t.box[3] + 2,
        )
      )
        labels[index] = -2
    }
  for (let start = 0; start < labels.length; start++)
    if (labels[start] === -1) {
      const id = components.length,
        component = { minX: source.width, minY: source.height, maxX: 0, maxY: 0, count: 0 }
      front = 0
      back = 1
      componentQueue[0] = start
      labels[start] = id
      while (front < back) {
        const index = componentQueue[front++]!,
        ...
        for (let dy = -1; dy <= 1; dy++)
          for (let dx = -1; dx <= 1; dx++) {
            ...
            if (
              ...
              labels[next] = id
              componentQueue[back++] = next
            }
          }
      }
      components.push(component)
    }
  const props: BlockNode[] = [],
    usedComponents = new Set<number>()
  for (const candidate of [...variant.candidates, ...(variant.rejectedCandidates ?? [])]) {
    ...
    const minX = Math.max(0, Math.floor(Math.min(...candidate.outer.map((p) => p[0])))),
      maxX = Math.min(source.width - 1, Math.ceil(Math.max(...candidate.outer.map((p) => p[0]))))
    const minY = Math.max(0, Math.floor(Math.min(...candidate.outer.map((p) => p[1])))),
      maxY = Math.min(source.height - 1, Math.ceil(Math.max(...candidate.outer.map((p) => p[1]))))
    for (let y = minY; y <= maxY; y++)
      for (let x = minX; x <= maxX; x++) {
        const id = labels[y * source.width + x]!
        if (id >= 0 && inside(x + 0.5, y + 0.5, candidate.outer))
    ...
    const best = [...hits]
      ...
      .find(([id, hit]) => {
        ...
        return (
          ...
          Math.max(w, d) * mpp <= 4 &&
          Math.max(w / d, d / w) < 8 &&
          !usedComponents.has(id)
        )
      })
    if (!best) continue
    const component = components[best[0]]!,
    ...
    const height = /bed|sofa/i.test(name) ? 0.65 : /coffee/i.test(name) ? 0.45 : 0.85
    const prop = BlockNode.parse({
      name: `Approximate ${name}`,
      parentId: levelId,
      position: [cx * mpp, 0, cy * mpp],
      topology: createBoxBlockTopology(
        (component.maxX - component.minX + 1) * mpp,
        height,
        (component.maxY - component.minY + 1) * mpp,
      ),
      metadata: {
        nativeMaskPreview: {
          role: 'prop',
          ...
          zoneId: zone.id,
          label: name,
          derivation: 'review-class-and-source-ink-component',
  ...
  for (let id = 0; id < components.length; id++) {
    ...
    const component = components[id]!,
      ...
      d = component.maxY - component.minY + 1
    ...
    const coverage = (vertical: boolean, fixed: number, from: number, to: number) => {
      ...
      for (let at = from; at <= to; at++) {
        ...
        for (let shift = -1; shift <= 1; shift++) {
          const x = vertical ? fixed + shift : at,
            y = vertical ? at : fixed + shift
          if (
            x >= 0 &&
            y >= 0 &&
            x < source.width &&
            y < source.height &&
            labels[y * source.width + x] === id
          )
            hit = true
        }
        if (hit) hits++
    ...
    if (
      Math.min(
        coverage(true, component.minX, component.minY, component.maxY),
        coverage(true, component.maxX, component.minY, component.maxY),
        coverage(false, component.minY, component.minX, component.maxX),
        coverage(false, component.maxY, component.minX, component.maxX),
      ) < 0.55
    )
      continue
    ...
    if (!zone) continue
    const prop = BlockNode.parse({
      name: `Furniture footprint · ${zone.name}`,
      parentId: levelId,
      position: [cx * mpp, 0, cy * mpp],
      topology: createBoxBlockTopology(w * mpp, 0.65, d * mpp),
      metadata: {
        nativeMaskPreview: {
          role: 'prop',
          approximation: true,
          sourceIds: [`ink-component:${id}`],
          zoneId: zone.id,
          derivation: 'source-enclosed-rectangular-symbol',
          classification: 'unclassified-furniture',
          heightSource: 'assumed',
        },
      },
    })
