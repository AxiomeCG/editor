...
test('repair flags preserve supported walls and apertures as approximations, without inventing missing features', async () => {
  ...
  expect(
    preview.mappings.find((mapping) => mapping.sourceId === 'left')?.nodeIds.length,
  ).toBeGreaterThan(0)
...
function covers(preview: NativeMaskPreview, point: [number, number]) {
  return Object.values(preview.nodes).some((wall) => {
    ...
    const across =
      Math.abs((point[0] - wall.start[0]) * dy - (point[1] - wall.start[1]) * dx) / length
    return along >= -1e-6 && along <= length + 1e-6 && across <= (wall.thickness ?? 0.1) / 2 + 1e-6
  })
}
...
test('short orthogonal wall returns survive without filling the adjacent room', async () => {
  ...
  const preview = await buildNativeMaskPreview(variant, source, options)
  expect(covers(preview, [0.4, 0.48])).toBe(true)
  expect(covers(preview, [1.2, 0.48])).toBe(false)
})
...
test('near-collinear long runs share an axis across a window but do not move a separate parallel wall', async () => {
  ...
  expect(Math.abs(host.end[0] - host.start[0])).toBeLessThan(0.02)
  expect(covers(preview, [1.44, 1])).toBe(true)
  expect(covers(preview, [0.8, 1])).toBe(false)
})
...
test('nearby openings receive a support pier, while a larger unrelated gap stays empty', async () => {
  ...
  variant.candidates = [
    rectangle('door', 'door', 10, 45, 30, 51),
    rectangle('window', 'window', 37, 45, 65, 51),
    rectangle('far', 'window', 82, 45, 98, 51),
  ]
  const preview = await buildNativeMaskPreview(variant, source, options)
  expect(preview.counts.doors).toBe(1)
  expect(preview.counts.windows).toBe(2)
  expect(covers(preview, [0.67, 0.96])).toBe(true)
  expect(covers(preview, [1.47, 0.96])).toBe(false)
})
...
test('a window mask wrapping a corner is trimmed to the jamb instead of dropping its aperture', async () => {
  ...
  const window = Object.values(preview.nodes).find((n) => n.type === 'window')
  expect(window).toBeDefined()
  expect(window?.width).toBeGreaterThan(0.65)
  expect(window?.width).toBeLessThan(0.9)
})

test('source text separates named open-plan zones without inserting a partition wall', async () => {
  const { variant, source, options, rectangle } = fixture()
  variant.candidates = [
    rectangle('top', 'wall', 10, 10, 90, 16),
    rectangle('bottom', 'wall', 10, 84, 90, 90),
    rectangle('left', 'wall', 10, 10, 16, 90),
    rectangle('right', 'wall', 84, 10, 90, 90),
  ]
  const preview = await buildNativeMaskPreview(
    variant,
    {
      ...source,
      labels: [
        { text: 'Kitchen', confidence: 1, box: [25, 45, 35, 55] },
        { text: 'Dining', confidence: 1, box: [65, 45, 75, 55] },
      ],
    },
    options,
  )
  ...
  const kitchen = zones.find((zone) => zone.name === 'Kitchen')!,
    dining = zones.find((zone) => zone.name === 'Dining')!
  expect(Math.max(...kitchen.polygon.map((p) => p[0]))).toBeLessThanOrEqual(
    Math.min(...dining.polygon.map((p) => p[0])) + 0.03,
  )
...
test('a review region recovers a window only when the source contains separated frame rails', async () => {
  ...
  variant.missingFeatures = [
    { kind: 'window', sourceRegion: [43, 35, 57, 65], evidence: 'Inspect this aperture.' },
  ]
  source.rgba.fill(255)
  expect((await buildNativeMaskPreview(variant, source, options)).counts.windows).toBe(0)
  for (let y = 35; y <= 65; y++)
    for (const x of [45, 46, 54, 55]) {
  ...
  expect(window?.width).toBeGreaterThan(0.5)
  for (let y = 35; y <= 65; y++)
    for (let x = 45; x <= 55; x++) {
      const index = (y * source.width + x) * 4
      source.rgba[index] = source.rgba[index + 1] = source.rgba[index + 2] = 0
...
test('a corner-glazing mask can produce two hosted windows when both source frame runs are present', async () => {
  ...
  for (let y = 25; y <= 78; y++)
    for (const x of [10, 11, 17, 18]) {
      const index = (y * source.width + x) * 4
      source.rgba[index] = source.rgba[index + 1] = source.rgba[index + 2] = 0
    }
  for (let x = 10; x <= 70; x++)
    for (const y of [70, 71, 77, 78]) {
      const index = (y * source.width + x) * 4
      source.rgba[index] = source.rgba[index + 1] = source.rgba[index + 2] = 0
    }
  const preview = await buildNativeMaskPreview(variant, source, options)
...
test('a genuinely oblique wall is not flattened onto the main rectangular grid', async () => {
  const { variant, source, options, rectangle } = fixture()
  const angle = Math.PI / 9
  const outer: ComparisonCandidate['outer'] = [
    [30, 40],
    [75, 40],
    [75, 45],
    [30, 45],
  ].map(([x, y]) => [
    50 + (x! - 50) * Math.cos(angle) - (y! - 50) * Math.sin(angle),
    50 + (x! - 50) * Math.sin(angle) + (y! - 50) * Math.cos(angle),
  ])
  variant.candidates = [
    rectangle('main', 'wall', 10, 10, 90, 16),
    { id: 'oblique', class: 'wall', outer, holes: [] },
  ]
  const preview = await buildNativeMaskPreview(variant, source, options)
  const ids = preview.mappings.find((mapping) => mapping.sourceId === 'oblique')!.nodeIds
  const walls = ids
    .map((id) => preview.nodes[id]!)
  ...
  const wall = walls[0]!
  expect(Math.atan2(wall.end[1] - wall.start[1], wall.end[0] - wall.start[0])).toBeCloseTo(angle, 2)
})
