...
export function mountNativePreview(container, data, status) {
  ...
  renderer.domElement.setAttribute(
    'aria-label',
    'Reconstructed Pascal room. Drag to orbit; wheel to zoom; arrow keys to pan.',
  )
  ...
  for (const part of data.meshes) {
    ...
    const materials = part.materials.map(
      (value) =>
  ...
  grid.position.set(center.x, -0.03, center.z)
  scene.add(grid)
  const camera = new PerspectiveCamera(42, 1, 0.01, span * 20)
  const controls = new OrbitControls(camera, renderer.domElement)
  light.position.copy(center).add(new Vector3(span * 0.6, span * 1.2, span * 0.4))
  ...
  const readyMessage = `3D ready · ${new Set(data.meshes.map((part) => part.nodeId)).size} native objects. Drag to orbit; wheel to zoom.`
    ...
    if (!frame && !disposed)
      frame = requestAnimationFrame(() => {
        ...
        try {
          ...
          status(`3D rendering failed: ${error.message}`)
        }
      })
  }
  const reset = (top = false) => {
    ...
    camera.position
      .copy(center)
      .add(top ? new Vector3(0, span * 1.5, 0.001) : new Vector3(span * 0.75, span * 1.1, span))
  ...
  const resize = () => {
    ...
    reportReady = true
    status('Rendering the native 3D view…')
    renderer.setPixelRatio(Math.min(devicePixelRatio, 2))
    renderer.setSize(width, height)
    camera.aspect = width / height
    camera.fov =
      (2 * Math.atan(Math.tan((21 * Math.PI) / 180) / Math.min(1, camera.aspect)) * 180) / Math.PI
  ...
  const pointerUp = (event) => {
    ...
    status(
      hit
        ? `${hit.object.userData.name} · native ${hit.object.userData.kind}`
        : 'Drag to orbit · wheel to zoom · click a primitive to identify it.',
    )
  }
  ...
  return {
    ...
    cutaway(enabled) {
      for (const material of wallMaterials) {
        material.clippingPlanes = enabled ? [new Plane(new Vector3(0, -1, 0), 1.1)] : []
