...
export function FloorplanImportDialog({ open, level, target, targetError, floorplanImport, restoreFocusRef, onOpenChange, onApplied }: {
  ...
  const measuredScale = useMemo(() => {
    ...
    const scale = measuredPlanScale(points, Number(distance))
    return scale !== null && scale >= .0001 && scale <= .2 ? scale : null
  }, [points, distance])
  useEffect(() => {
  ...
  return <Dialog open={open} modal={false} onOpenChange={onOpenChange}>
    <DialogContent className="top-20 right-4 left-auto max-h-[calc(100vh-6rem)] w-[360px] max-w-[calc(100vw-2rem)] translate-x-0 translate-y-0 gap-4 overflow-y-auto p-4 sm:max-w-[360px]"
      ...
      <DialogHeader>
        ...
        <DialogDescription>{target?.levelName ?? level.name ?? 'Selected floor'}</DialogDescription>
      </DialogHeader>
      <div className="rounded-lg border bg-muted/30 p-3">
        <div className="flex items-center justify-between gap-2 text-xs"><span className="font-medium">Prepared example</span><span className="text-muted-foreground">Ready to import</span></div>
      ...
      {source && <>
        ...
        <Button type="button" disabled={!target || !!busy || (points.length > 0 && !measuredScale)} onClick={event => void generate(event.detail !== 0)}>
          ...
          {busy === 'generate' ? 'Building native geometry…' : reconstruction ? 'Regenerate preview' : 'Preview floorplan'}
        </Button>
