...
export function buildLevelDuplicateCreateOps({
  ...
  const keptReferences = new Map([...idMap].filter(([, id]) => keptIds.has(id as AnyNodeId)))
  const cleanedNodes = filteredNodes.map((node) => {
    node = remapRepeatedReferences(node, keptReferences)
    if (node.type === 'zone')
      node = {
        ...node,
        name: renumberUnitName(node.name, floorNumber(level), floorNumber(level) + 1)!,
      }
    for (const key of [
      'arrayModifier',
      'linkedArray',
    ...
    if (reference)
      node = {
        ...
        metadata: {
          ...node.metadata,
          referenceOutline: { ...reference, linked: false, detachedReason: 'source-not-copied' },
        },
      }
  ...
  if (parentBuildingId && nodes[parentBuildingId]?.type === 'building') {
    for (const unit of Object.values(nodes)) {
      if (
        ...
        continue
      const copied = UnitNode.parse({
        ...unit,
        id: undefined,
        parentId: parentBuildingId,
        name: renumberUnitName(unit.name, floorNumber(level), floorNumber(level) + 1),
        members: unit.members.map((id) => idMap.get(id)),
        metadata: {
          ...unit.metadata,
          ...(unit.metadata.referenceOutline
            ? {
                referenceOutline: {
                  ...(unit.metadata.referenceOutline as Record<string, unknown>),
                  linked: false,
                  detachedReason: 'source-not-copied',
                },
              }
            : {}),
        },
      })
      for (const key of ['arrayModifier', 'linkedArray', 'arrayError']) delete copied.metadata[key]
      cleanedNodes.push(copied)
    }
  }
      ...
      node:
        node.id === newLevelId
          ? ({
              ...
              name: repeatedFloorName(level, nextLevelNumber),
            } as AnyNode)
          : node,
      parentId:
        node.id === newLevelId
